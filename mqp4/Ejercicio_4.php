...
function numero_max($numero){
/*Definimos el número máximo de digitos en un numero.
input: int - numero
output:int - numero máximo
*/
    $contador = 0;
    $max=$contador; 
    $numero_string= strval($numero);
...
function calcula_caracteres($matriz,$numero_maximo){
/*calculamos el número máximo de caracteres que ocupa la matriz
input: $matriz: array - Matriz predefinida;  $numero_maximo: int - Número máximo de digitos.
output:int - numero de caracteres
*/
    $tamaño = count($matriz[0]);
    $caracteres= ($numero_maximo * $tamaño) + 5;
    return $caracteres;
};

function imprimir_matriz_formateada($matriz){
    /*Recibida una matriz, imprime cada elemento y añade formato
    input: array - Matriz
    output: 
    */ 
    //Calculamos el número máximo de la matriz a imprimir
    for($i=0; $i<count($matriz); $i++){ 
        for($j=0; $j<count($matriz); $j++){
            $valor_max=numero_max($matriz[$i][$j]);
            };
        };
    //usamos la función de calcular caracteres
    $caracteres_totales = calcula_caracteres($matriz,$valor_max);
    //imprimimos la parte superior de la capselera
    echo "--";
    echo (str_repeat(" ",($caracteres_totales-4)));
    echo"--\n";
    //imprimimos nuestra matriz formateada
    for($i=0; $i<count($matriz); $i++){ 
        echo "|";
        for($j=0; $j<count($matriz); $j++){
            if ($j!=count($matriz)-1){
                echo str_pad($matriz[$i][$j], $valor_max,0,STR_PAD_LEFT);
                echo ",";
            }
            else{
                echo str_pad($matriz[$i][$j], $valor_max,0,STR_PAD_LEFT);
    ...
    //Imprimimos la parte inferior de la capselera
        echo "--";
        echo (str_repeat(" ",($caracteres_totales-4)));
        echo"--";
    };
...
switch($opcion){
        ...
        case 4:
            ...
            for ($i=0;$i<count($matriz_de_matrices);$i++){
                echo("Matriz_".($i+1).("\n"));
                imprimir_matriz_formateada($matriz_de_matrices[$i]);
                echo "\n";
                };
            break;
